...
public class AppShowBehaviorServiceImpl implements AppShowBehaviorService {
    ...
    @Override
    public ResponseResult saveShowBehaivor(ShowBehaviorDto dto) {
        //获取用户信息。设备id
        ApUser user = AppThreadLocalUtils.getUser();
        //根据当前的用户id和设备id查询行为实体
        ...
        if(!articleIdList.isEmpty()) {
            articleIds = new Integer[articleIdList.size()];
            articleIdList.toArray(articleIds);
